...
exports.offlineIndexDbExchange = void 0;
//@ts-nocheck
...
function offlineIndexDbExchange({ cacheFirst, schemaObject, resolvers, writeHooks, }) {
    const typeInfo = new graphql_1.TypeInfo(schemaObject);
    async function onOperation(op) {
        ...
        if (cacheFirst) {
            if (op.kind === 'query') {
                op.cacheResult = await (0, graphql_1.execute)({
                    schema: schemaObject,
                    document: op.query,
                    rootValue: resolvers,
                    contextValue: {
    ...
    async function onResult(bubble) {
        ...
        // if its network-first and we get a network error, use fetch offline cache
            ...
            if (bubble.error) {
                bubble.data = (await (0, graphql_1.execute)({
                    schema: schemaObject,
                    document: op.query,
                    rootValue: resolvers,
                    contextValue: {
        ...
        const typeMap = {};
        (0, graphql_1.visit)(op.query, (0, graphql_1.visitWithTypeInfo)(typeInfo, {
            Field: {
    ...
    return function exchange({ forward }) {
        return (operations) => {
            return (0, wonka_1.pipe)((0, wonka_1.pipe)(operations, (0, wonka_1.mergeMap)((operation) => {
                const newOperation = (onOperation && onOperation(operation)) || operation;
                return 'then' in newOperation
                    ? (0, wonka_1.fromPromise)(newOperation)
                    : (0, wonka_1.fromValue)(newOperation);
            })), forward, (0, wonka_1.mergeMap)((result) => {
                ...
                return 'then' in newResult
                    ? (0, wonka_1.fromPromise)(newResult)
                    : (0, wonka_1.fromValue)(newResult);
            }));
        };
    };
}
exports.offlineIndexDbExchange = offlineIndexDbExchange;
            ...
            if (typeof value === 'object' && value !== null) {
                ...
                if (!Array.isArray(value)) {
                    ...
                    if (writeHooks?.[tableName]) {
                        ...
                        const cleanedValue = Object.fromEntries(Object.entries(value).filter(([key, v]) => {
                        ...
                        try {
                            await writeHooks[tableName](parent, cleanedValue, { db: db_1.db }, { objType });
                        }
                        catch (e) {
                            console.error(e);
                        }
