...
function journalItem({ selected = false, apiaryId, data, hiveId, added, id, }) {
    ...
    return (<div className={`${styles_less_1.default.journalItem} ${selected ? styles_less_1.default.journalItemSelected : ''}`}>
			<link_1.default className={null} href={`/apiaries/${apiaryId}/hives/${hiveId}/inspections/${id}`}>
				<dateFormat_1.default datetime={added}/>
				<div className={styles_less_1.default.journalItemStats}>
					<div style={`background-color:${colors_1.default.broodColor};height:${stats.brood}px;`}></div>
					<div style={`background-color:${colors_1.default.honeyColor};height:${stats.honey}px;border-top:1px solid #ffAA00;`}></div>
				</div>
			</link_1.default>
		</div>);
}
