...
describe('Hive Log Model', () => {
	beforeEach(() => {
		...
		dbMocks.mockBulkDelete.mockReset()
		...
		dbMocks.mockBulkDelete.mockResolvedValue(undefined)
	...
	it('addHiveLog should prune old local logs and retry when IndexedDB quota is exceeded', async () => {
		const quotaError = new DOMException('QuotaExceededError', 'AbortError')
		dbMocks.mockAdd.mockRejectedValueOnce(quotaError).mockResolvedValueOnce(42)
		dbMocks.mockSortBy.mockResolvedValue(
		...
		expect(dbMocks.mockAdd).toHaveBeenCalledTimes(2)
		expect(dbMocks.mockBulkDelete).toHaveBeenCalledWith([151, 152])
	})

	it('addHiveLog should still call backend when IndexedDB quota recovery fails', async () => {
		const quotaError = new DOMException('QuotaExceededError', 'AbortError')
		dbMocks.mockAdd.mockRejectedValue(quotaError)
		dbMocks.mockSortBy.mockResolvedValue([])
		;(fetch as Mock).mockResolvedValue(
			new Response(
				JSON.stringify({
	...
	it('formatHiveLogAction should return expected labels and fallback', () => {
		expect(formatHiveLogAction(hiveLogActions.STRUCTURE_ADD)).toBe(
			'Structure added'
		)
		expect(formatHiveLogAction(hiveLogActions.LOCATION_MOVE)).toBe(
			'Location changed'
		)
	...
	it('syncHiveLineageLogs should create lineage entries for merge relations', async () => {
		...
		expect(payloads.every((row) => row.action === hiveLogActions.LINEAGE)).toBe(
			true
		)
	})
})
